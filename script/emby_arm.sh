...
case $cpu_arch in
        ...
        "aarch64" | *"arm64"* | *"armv8"* | *"arm/v8"*)
            ...
            docker pull amilys/embyserver_arm64v8:$version
            ;;
        *)
...
case $cpu_arch in
	...
	"aarch64" | *"arm64"* | *"armv8"* | *"arm/v8"*)
		if [ $mode == "host" ]; then	
	        docker run -d --name emby -v $media_dir/config:/config -v $media_dir/xiaoya:/media -v $config_dir/nsswitch.conf:/etc/nsswitch.conf --network=host --add-host="xiaoya.host:$xiaoya_host" $gpu --user 0:0 -v UID=0 -v GID=0 -v GLIST=0 --restart always amilys/embyserver_arm64v8:$version
		fi
		if [ $mode == "bridge" ]; then	
	        docker run -d --name emby -v $media_dir/config:/config -v $media_dir/xiaoya:/media -v $config_dir/nsswitch.conf:/etc/nsswitch.conf -p 6908:6908 --add-host="xiaoya.host:$xiaoya_host" $gpu --user 0:0 -v UID=0 -v GID=0 -v GLIST=0 --restart always amilys/embyserver_arm64v8:$version
		fi
		echo "一键全家桶全部安装完成"
        ;;
	*)
